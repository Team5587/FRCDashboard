...
function createWindow() {
    ...
    if (process.platform === 'win32') {
        ...
        server = require('child_process').spawn('py', ['-3', '-m', 'pynetworktables2js','--robot=10.55.87.2']);
    } else {
        // If on unix-like/other OSes, use bash command (python3 ./server.py).
        server = require('child_process').spawn('python3', ['-m', 'pynetworktables2js','--robot=10.55.87.2']);
    }
